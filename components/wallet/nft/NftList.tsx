...
const NftList = ({ nftData, selectNft, setSelectNft }: any) => {
  const { selectTabViewValue } = useWalletTabValue();

  return (
    <div className='max-w-full h-[700px] overflow-y-scroll'>
      {selectTabViewValue === 'walletList' ? (
        <div className='flex flex-col gap-2 relative z-10 p-2'>
          {nftData?.result.map((item: any, index: number) => (
            <div
              key={index}
              className={`px-4 py-2 rounded-lg shadow-medium flex items-center  justify-between bg-white gap-5 ${
            ...
            >
              ...
              <div className='flex flex-col w-full'>
                <div className='flex items-center justify-between'>
                  ...
                  <div className='text-sm text-gray-500 text-ellipsis overflow-hidden mt-2'>
