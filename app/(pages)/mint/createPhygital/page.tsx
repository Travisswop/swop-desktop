  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/desktop/nft/phygitals`,
        formData,
        config
      );

      if (response.data.state === "success") {
        alert("Phygital item created successfully!");
        setFormData({
          name: "",
          description: "",
          imageUrl: "",
      ...
      console.error("Error creating phygital item:", error);
      alert("Failed to create phygital item");
    }
  };
